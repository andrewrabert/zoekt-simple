...
func listRepos(baseURL, query string) (*listResponse, error) {
	...
	resp, err := searchHTTPClient.Post(u, "application/json", bytes.NewReader(data))
	if err != nil {
		return nil, err
	}
...
func doSearch(baseURL, query string, maxResults, contextLines int) (*searchResponse, error) {
	body := map[string]any{
		"Q": query,
		"Opts": map[string]any{
...
Query syntax:
  ...
  case:yes - case-sensitive search`,
	...
	cmd.Flags().IntVarP(&maxResults, "max-results", "n", 50, "Maximum number of file matches")
	cmd.Flags().BoolVar(&jsonOutput, "json", false, "Output results as JSON")
	cmd.Flags().StringVar(&outputMode, "output-mode", "lines", "Output format: lines, files, repos")
	cmd.Flags().BoolVarP(&filesWithMatches, "files-with-matches", "l", false, "Only print file paths with matches")
	cmd.Flags().IntVarP(&contextLines, "context", "C", 0, "Context lines around matches")
	cmd.Flags().BoolVar(&column, "column", false, "Show column numbers")
	cmd.Flags().BoolVar(&vimgrep, "vimgrep", false, "file:line:column:content format")
	cmd.Flags().StringVar(&headingMode, "heading", "auto", "Heading mode: auto, always, never")
	cmd.Flags().BoolVarP(&noLineNumber, "no-line-number", "N", false, "Suppress line numbers")
	cmd.Flags().StringVar(&color, "color", "auto", "Color output: auto, always, never")
...
func runSearch(opts searchOpts) error {
	...
	lineNumber := !opts.noLineNumber

	// Auto-detect repo-only queries and use the list endpoint
	if opts.outputMode == "lines" && isRepoOnlyQuery(opts.query) {
		opts.outputMode = "repos"
	}

	if opts.outputMode == "repos" && isRepoOnlyQuery(opts.query) {
		listResp, err := listRepos(opts.url, opts.query)
		if err != nil {
			return err
		}
	...
	resp, err := doSearch(opts.url, opts.query, opts.maxResults, opts.contextLines)
	if err != nil {
		return err
	}
	...
	var result string
	if opts.jsonOutput {
		result = formatJSON(resp, opts.outputMode)
	} else if opts.outputMode == "repos" || opts.outputMode == "files" {
		result = formatPlain(resp, opts.outputMode)
	} else {
		useHeading := opts.headingMode == "always" || (opts.headingMode == "auto" && isTerminal() && !opts.vimgrep)
		useColor := opts.color == "always" || (opts.color == "auto" && isTerminal() && os.Getenv("NO_COLOR") == "")

		result = formatRipgrep(resp, ripgrepOpts{
			heading:        useHeading,
			contextLines:   opts.contextLines,
			showColumn:     opts.column || opts.vimgrep,
			showLineNumber: lineNumber,
			useColor:       useColor,
			vimgrep:        opts.vimgrep,
			filesOnly:      opts.filesWithMatches,
		})
	}
...
// RunSearch is exported so the legacy zoekt-search binary can delegate to it.
